...
// Popping Sidebar
const menuIcon = document.querySelector(".hamburger-menu.hidden");
...
// For swiggling each char of Popping Side NavBar
const sidebarEl = document.querySelectorAll(".nav .list li a");

sidebarEl.forEach((el) => {
  el.innerHTML = el.innerText
    .split("")
    .map(
      (letter, idx) =>
        `<span class="rot-sidebar-char" style="transition-delay: ${
          idx * 50
        }ms;">${letter}</span>`
    )
    .join(" ");
